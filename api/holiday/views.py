...
#         user_employment_date = UserMaster.objects.get(id=user_id)
#
...
class HolidayCheckView(ListView):
    ...
    def get_original_queryset(self):
        current_date = datetime.now().date()
        user_id = self.request.user.id
        user_employment_date = UserMaster.objects.get(id=user_id).employment_date
        replace_employment_date = (user_employment_date.replace(year=timezone.now().year)) + timedelta(days=1)
        next_replace_employment_date = user_employment_date.replace(year=timezone.now().year) + relativedelta(years=1)
        print('replace_employment_date :', replace_employment_date)
        print('last next_replace_employment_date :', next_replace_employment_date)
