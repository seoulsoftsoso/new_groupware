...
from api.models import ItemIn, ItemInPay
from lib import Pagenation


class Order_purchase_read(View):
    def get(self, request, *args, **kwargs):
        ...
        # 단가가 0 이상인 경우만
        qs = qs.filter(~Q(in_price=0))

        # Search
        if make_from != '':
            qs = qs.filter(in_at__gte=make_from)

        if make_to != '':
            qs = qs.filter(in_at__lte=make_to)
        ...
        # 매입 합계
        total = qs.aggregate(
            receive_amount_total=Coalesce(Sum('receive_amount'), 0),
            in_faulty_amount_total=Coalesce(Sum('in_faulty_amount'), 0),
            in_price_total=Coalesce(Sum('in_price'), 0),  # 단가
        )
        receive_amount_total = total['receive_amount_total']

        # 소계 - 품번별
        # qs_sorted = qs.values('item_id').annotate(
        #     receive_amount_sum=Sum('receive_amount'),
        #     in_faulty_amount_sum=Sum('in_faulty_amount'),
        #     in_price_sum=Sum('in_price'),  # 입고단가
        # ).order_by('item_id')

        # 소계 - 거래처별
        qs_sorted = qs.values('customer').annotate(
            receive_amount_sum=Sum('receive_amount'),
            in_faulty_amount_sum=Sum('in_faulty_amount'),
            in_price_sum=Sum('in_price'),  # 입고단가
        ).order_by('customer')
        ...
        total_supply_sum = 0  # 매입합계 - 공급가 합
        total_surtax_sum = 0  # 매입합계 - 부가세 합
        ...
        if qs_sorted:
            for row in qs_sorted:
                ...
                _purchase_from = row['customer']
                sqs = qs.filter(customer=_purchase_from).order_by('id')
                ...
                appendList({
                    'type': 'sum',
                    'in_amount_sum': tof(row['receive_amount_sum'] - row['in_faulty_amount_sum'], 3),  # 입고수량
                    'in_price_sum': row['in_price_sum'],  # 입고단가
        ...
        # 합계 계산
        total_in_amount = tof(total['receive_amount_total'] - total['in_faulty_amount_total'], 3)
        total_in_price = total['in_price_total']
        # total_supply = tof((total['receive_amount_total'] - total['in_faulty_amount_total']) * (total['in_price_total']), 3)
        # total_surtex = tof(((total['receive_amount_total'] - total['in_faulty_amount_total']) * (total['in_price_total']))/10, 3)
        # total_supply_surtex = total_supply + total_surtex

        context['total_in_amount'] = total_in_amount  # 수량
        context['total_in_price'] = total_in_price # 단가
        context['total_supply'] = tof(total_supply_sum, 3)  # 공급가 합
        context['total_surtex'] = tof(total_surtax_sum, 3)  # 부가세 합
        context['total_supply_surtex'] = tof(total_supply_sum + total_surtax_sum, 3)  # 합계금액
...
def get_results(qs):
    ...
    for row in qs.object_list:
        ...
        appendResult({
            ...
            'in_amount': tof(row.in_amount, 3),  # 입고수량
            'in_price': tof(row.in_price, 3),  # 입고단가

            'supply': tof(row.in_amount * row.in_price, 3),  # 공급가액 = 수량 * 단가
            'surtax': tof(row.in_amount * row.in_price * 0.1, 3),  # 부가세액 = 공급가액 * 0.1
            'sum': tof((row.in_amount * row.in_price) + (row.in_amount * row.in_price * 0.1), 3),  # 합계금액 = 공급가액 + 부가세액
            'in_price': tof(row.in_price, 3),  # 지급일자
            'etc': row.etc,  # 비고
        })
...
def get_results_list(qs):
    ...
    for row in qs:
        if row.customer:
            from_id = row.customer.id
            from_name = row.customer.name
        else:
            from_id = ''
            from_name = ''

        iqs = ItemInPay.objects.filter(item_in_id=row.id).order_by('id')
        ...
        supply = tof(row.in_amount * row.in_price, 3)
        surtax = tof(row.in_amount * row.in_price * 0.1, 3)
        sum = tof(supply + surtax, 3)
        remain_amount = tof(sum - pay_amount, 3)

        pay_div = '미지급'  # '부분지급' '지급완료'
        if pay_amount > 0:
            pay_div = '부분지급'
            if pay_amount >= sum:
                pay_div = '지급완료'
...
def get_results2(qs):
    ...
    for row in qs.object_list:
        if row['type'] == 'list':
            ...
            appendResult({
                'type': 'sum',
                'in_amount_sum': row['in_amount_sum'],
                'in_price_sum': row['in_price_sum'],
