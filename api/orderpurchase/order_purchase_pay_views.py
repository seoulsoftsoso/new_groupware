...
from api.models import ItemIn, ItemOut, ItemInPay, UserMaster
from lib import Pagenation
from msgs import msg_error, msg_pk, msg_delete_fail


class Order_purchase_pay_create(View):
    ...
    def post(self, request):
        ...
        item_in_id = int(request.POST.get('item_in_id', ''))  # 입고 리스트 ID
...
class Order_purchase_pay_read(View):
    def get(self, request, *args, **kwargs):
        item_in_id = request.GET.get('item_in_id', '')
        if item_in_id == '':
            item_in_id = 0

        item_in_id = int(item_in_id)
        qs = ItemInPay.objects.filter(enterprise__name=request.COOKIES['enterprise_name']).order_by('id')
        qs = qs.filter(item_in_id=item_in_id)
...
class Order_purchase_pay_update(View):
    ...
    def post(self, request):
        ...
        try:
            with transaction.atomic():
                obj = ItemInPay.objects.get(pk=pay_click_id)
...
class Order_purchase_pay_delete(View):
    ...
    def post(self, request):
        ...
        try:
            with transaction.atomic():
                obj = ItemInPay.objects.get(pk=int(pk))
                obj.delete()
